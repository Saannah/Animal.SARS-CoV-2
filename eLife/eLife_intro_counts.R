## create intro count table for elife
...
animal_2_human = data.frame(species = vector(),
                            replicate = vector(),
                            filtered_count = vector(),
                            unfiltered_count = vector())

for(species in c("cat", "dog", "mink", "deer")){
  for(number in 1:10){
    intros = find_intro(species, number, c("animal", "human"))
    animal_2_human[nrow(animal_2_human) + 1, ] = c(species, number,
                                                   length(unlist(intros$filtered_MRCAin)),
                                                   length(unlist(intros$unfiltered_MRCAin)))
  }
}


write.table(animal_2_human, file = "counts_animal_2_human.tsv", sep = "\t", quote = TRUE, row.names = FALSE)



human_2_animal = data.frame(species = vector(),
                            replicate = vector(),
                            filtered_count = vector(),
                            unfiltered_count = vector())
for(species in c("cat", "dog", "mink", "deer")){
  for(number in 1:10){
    intros = find_intro(species, number, c("human", "animal"))
    animal_2_human[nrow(animal_2_human) + 1, ] = c(species, number,
                                                   length(unlist(intros$filtered_MRCAin)),
                                                   length(unlist(intros$unfiltered_MRCAin)))
  }
...
write.table(animal_2_human, file = "counts_human_2_animal.tsv", sep = "\t", quote = TRUE, row.names = FALSE)
