...
find_intro = function(species, number, fromto){
  ...
  tree = read.tree(paste0("/home/sanna/projects/def-shapiro/sanna/aim1and2/eLife_review_Feb2023/ancestral_seq_iqtree/all_treefiles/",species,"_subtree_",number,"_trimmed.nwk"))
...
for(species in c("cat","dog","mink", "deer")){
  ...
  for(replicate in 1:10){
    ...
    transmission_msa = data.frame(out_node_number = character(),
                                  in_node_number = character(),
                                  
                                  out_node_label_iqtree = character(),
                                  in_node_label_iqtree = character(),
                                  
                                  out_sequence = character(),
                                  in_sequence = character(),
                                  
                                  ndiff = character(),
                                  sites = character())
    
    for(i in 1:length(unlist(intros$filtered_MRCAout))){
      out_node_number = unlist(intros$filtered_MRCAout)[i]
      in_node_number = unlist(intros$filtered_MRCAin)[i]
      out_node_label = out_names[i]
      out_sequence = msa$seq.text[which(msa$seq.name == out_node_label)]
      if(in_node_number <= length(intros$tree$tip.label)){
        in_node_label = intros$tree$tip.label[unlist(intros$filtered_MRCAin)[i]]
        in_sequence = all_msa$seq.text[which(all_msa$seq.name == in_node_label)]
      }
      if(in_node_number > length(intros$tree$tip.label)){
        in_node_label = unlist(str_split(intros$tree$node.label[(in_node_number - length(intros$tree$tip.label))], "/"))[1]
        in_sequence = msa$seq.text[which(msa$seq.name == in_node_label)]
      }
      
      out_seq_list = unlist(str_split(out_sequence, ""))
      in_seq_list = unlist(str_split(in_sequence, ""))
      n_diff = length(which(tolower(out_seq_list) != tolower(in_seq_list)))
      sites = paste0(which(tolower(out_seq_list) != tolower(in_seq_list)), collapse = "/")
        
      transmission_msa[nrow(transmission_msa)+1, ] = c(out_node_number, in_node_number,
                                          out_node_label, in_node_label,
                                          out_sequence, in_sequence, n_diff, sites)
    } #looping through transmissions ends here
    
    write.table(transmission_msa, file = paste0(species, "_", replicate, "_intro_sequences_table_animal2human.tsv"), quote = TRUE, row.names = FALSE, sep = "\t")
  }
}
