...
find_intro = function(species, number){
  ...
  #tree = read.tree(paste0("/Users/sana/Documents/Jan2022/plots_18032022/animal_to_human/",species,"_subtree_",number,"_trimmed.nwk"))
  #shuffle the tips:
  ...
  if(length(trans_nodes)==0){return(c(0,0))}
  ...
  if(length(trans_nodes) == 0){
    out_obj = c(length(unlist(MRCAin)), 0)
    return(out_obj)
  }
  ...
  # print("mrca")
  # print(MRCAin)
  out_obj = c(length(unlist(MRCAin)), length(unlist(MRCAin_filtered)))
  return(out_obj)
...
tic()
tab = mapply(find_intro, species, 1:1000)
toc()

tab = t(tab)
#write file
write.table(tab, file = paste0(species, "_", subtree, "permutations.tsv"), sep = "\t", row.names = FALSE, col.names = FALSE)